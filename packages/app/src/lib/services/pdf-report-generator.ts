...
export class PDFReportGenerator {
  ...
  static async generateBasicSummary(
    ...
    doc.text('Generated with EU Border Authority Free Account - Upgrade for detailed reports', 20, yPosition)
    yPosition += 15
  ...
  private static addHeader(doc: jsPDFWithPlugin, yPosition: number): number {
    ...
    doc.setFontSize(16)
    doc.setTextColor(this.COLORS.primary)
    doc.text('🛂 EU Border Authority', 25, yPosition + 8)
    
    doc.setFontSize(10)
    doc.setTextColor(this.COLORS.secondary)
    doc.text('Professional EU Border Compliance Report', 25, yPosition + 15)
  ...
  private static addLegalDisclaimer(doc: jsPDFWithPlugin, yPosition: number): number {
    ...
    const disclaimer = [
      ...
      '',
      'EU Border Authority provides this service to assist with travel planning but assumes',
  ...
  private static addFooter(doc: jsPDFWithPlugin, reportDate: Date): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      // Footer text
      doc.text('Generated by EU Border Authority Premium', 20, 285)
      doc.text(`Report Date: ${format(reportDate, 'dd/MM/yyyy HH:mm')}`, 20, 292)
      doc.text(`Page ${i} of ${pageCount}`, 170, 285)
      doc.text('euborder.com', 170, 292)
    }
  }
}
